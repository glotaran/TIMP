"plotFLIMresid" <- function(multimodel, multitheta, plotoptions, dind) {
  ...
  for (i in 1:length(m)) {
    ...
    svdresidlist[[length(svdresidlist) + 1]] <- doSVD(
      residuals,
      2, 2
    )
  ...
  if (increasing_x2) {
    ...
    image.plot(xpos, x2, residlist[[1]],
  ...
  if (nt > 1 && nl > 1) {
    matplot(x, svdresidlist[[1]]$left[, i],
      type = "l", main = "Left sing. vec. residuals ",
      ylab = "", log = "x", xlab = plotoptions@xlab, col = 1
    )
    if (length(m) > 1) {
      for (i in 2:length(m)) {
        matlines(m[[i]]@x, svdresidlist[[i]]$left,
          log = "x",
          type = "l", col = i
        )
      }
