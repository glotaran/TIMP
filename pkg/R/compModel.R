...
function (k, kinscal = vector(), x, irfpar = vector(), irf = FALSE, 
    ...
    kin2scal = vector(), reftau = 0, anispec = list(calcani=FALSE), 
    anipar = vector(), cohcol = vector()) 
{
    if (fullk) {
	...
	compModel(k=kinpar2, fullk = speckin2$fullk, kmat = speckin2$kmat,
	kinscal = kinscal, jvec = speckin2$jvec, x=x, irfpar=irfpar, irf=irf,
	lamb=lamb, dataset=dataset, mirf = mirf, measured_irf = measured_irf,
