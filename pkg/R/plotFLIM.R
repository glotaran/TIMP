"plotFLIM" <- function (multimodel, multitheta, plotoptions) 
{
   ...
   for(j in 1:ncol(csave)) { 
	 ...
	 if(c2)
	    if("#0000FF" %in% csave[,ncol(csave)-(j-1)]) {	
		colend <- ncol(csave)-(j-1) 
		c2 <- FALSE 
	    }
   }
   c1 <- c2 <- TRUE 
   for(j in 1:nrow(csave)) { 
	  if(c1) 
	      if("#0000FF" %in% csave[j,]) {	
	          rowstart <- j
		  c1 <- FALSE
	   ...
	   if(c2)     
	      if("#0000FF" %in% csave[nrow(csave)-(j-1),] ) {	
	        rowend <- nrow(csave)-(j-1)
	        c2 <- FALSE
	      }
   }
