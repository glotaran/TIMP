...
psi_1_full <- preProcess(psi_1,
  sel_time = c(178, 478),
  sel_lambda_ab = c(440, 640)
)

psi_1_sampled <- preProcess(psi_1,
  sel_time = c(178, 478),
...
res_polylin <- fitModel(
  data = list(psi_1_sampled),
  modspec = list(model_polylin),
  opt = specopt(
    iter = 7, linrange = 20, stderrclp = TRUE,
...
model_exp_linkedrates <- initModel(
  mod_type = "spec",
...
res_model_exp_linkedrates <- fitModel(
  data = list(psi_1_sampled),
  modspec = list(model_exp_linkedrates),
  opt = specopt(
    iter = 5, linrange = 20, residplot = TRUE,
